...
&emsp;&emsp;将阵列首个阵元视为参考阵元，则在 t 时刻，第 k 个信源使得首个阵元的接收信号为：<br/>
$$
\begin{equation}
S_{k} =  \mathcal{U}_{k}(t)exp(j(\omega_{0}t+\varphi(t))),k= 1,2,...,K \tag{2-1}\\
\end{equation}
$$
<br/>&emsp;&emsp;其中，$j$ 表示虚部单位，${U}_{k}(t)$、$\omega_{0}$、$\varphi(t)$表示接收信号的幅度、频率和相位。因此，在 t 时刻，第 m 个阵元的接收信号可表示为：<br/><br/>
$$
\begin{equation}
